...
- `test_zz_strict_actual_evidence_manifest.py`：session 级实际 evidence 审计，断言当前 strict suite 覆盖全部 Flow / LogicStep / AgentStep / submit tool 和 197 个本地 implemented application ToolCase；当前 ToolCase 表另有 37 个 pending fixture 和 13 个 env-gated 工具；同时审计 checkpointed write ToolCase 都有实际调用和断言摘要。
...
- 默认 strict suite 当前 registry surface 为 247 个 application tools，其中 197 个 ToolCase 标记 implemented，37 个 pending fixture，13 个 env-gated；implemented 部分审计 Flow / LogicStep / AgentStep / submit / checkpointed write evidence。
