...
- `test_zz_strict_actual_evidence_manifest.py`：session 级实际 evidence 审计，断言当前 strict suite 覆盖全部 Flow / LogicStep / AgentStep / submit tool 和 176 个本地 implemented application ToolCase；当前 ToolCase 表另有 18 个 pending fixture 和 8 个 env-gated 工具；同时审计 checkpointed write ToolCase 都有实际调用和断言摘要。
...
- 默认 strict suite 当前 registry surface 为 202 个 application tools，其中 176 个 ToolCase 标记 implemented，18 个 pending fixture，8 个 env-gated；implemented 部分审计 Flow / LogicStep / AgentStep / submit / checkpointed write evidence。
