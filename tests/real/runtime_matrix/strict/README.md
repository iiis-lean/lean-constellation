...
- `test_zz_strict_actual_evidence_manifest.py`：session 级实际 evidence 审计，断言当前 strict suite 覆盖全部 Flow / LogicStep / AgentStep / submit tool 和 195 个本地 implemented application ToolCase；当前 ToolCase 表另有 48 个 pending fixture 和 13 个 env-gated 工具；同时审计 checkpointed write ToolCase 都有实际调用和断言摘要。
