...
def test_repo_public_boundary_batches_all_main_exports(
    ...
    batches: list[tuple[list[DeclRef], ProofAvailability]] = []

    def check_refs_batch(_repo_root: Path, *, refs, local_target, **_kwargs):
        selected = list(refs)
        batches.append((selected, local_target))
        return runtime.foundation.ok(
            [
                DeclReadinessReport(
                    ...
                    ready=True,
                    summary=f"{ref.node}:{ref.name} is ready.",
                )
                for ref in selected
            ]
        )
    ...
    assert batches == [(refs, ProofAvailability.PROVED)]
