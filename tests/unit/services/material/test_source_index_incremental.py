...
from lean_constellation.services.material import SourceIndexComponent
...
def test_scoped_initial_update_requires_owner_and_ignores_unselected_pending(tmp_path: Path) -> None:
    ...
    bypass = runtime.material.commit_source_index(tmp_path)
    assert not bypass.ok and bypass.issues[0].kind == "source_index_committed"
...
def test_append_only_update_rejects_old_payload_change_and_baseline_drift(tmp_path: Path) -> None:
    ...
    legacy_commit = runtime.material.commit_source_index(tmp_path)
    assert not legacy_commit.ok and legacy_commit.issues[0].kind == "source_index_update_context_required"
...
def test_v2_inspection_is_read_only_and_committed_migration_rolls_back_on_verify_failure(
    ...
    v2 = {
        ...
        "blocks": {
            ...
            "b_0001": {
                ...
                "summary": "Legacy theorem source.",
                "lifecycle_status": "completed",
                "refs": [{
                    "ref_id": "ref_0001",
                    "material_ref": {"kind": "source", "ref": {"path": "chapters/one.md", "start_line": 1, "end_line": 2}},
                    "role": "primary",
                }],
                "link_ids": [],
                "child_ids": [],
        ...
        "links": {},
        "files": {
            "README.md": {"path": "README.md", "line_count": 6, "readable_text": True, "survey_status": "surveyed", "indexing_status": "indexed", "summary": None},
            "artifact.bin": {"path": "artifact.bin", "line_count": 0, "readable_text": False, "survey_status": "pending", "indexing_status": "pending", "summary": None},
            "chapters/one.md": {"path": "chapters/one.md", "line_count": 2, "readable_text": True, "survey_status": "surveyed", "indexing_status": "indexed", "summary": None},
            "chapters/two.md": {"path": "chapters/two.md", "line_count": 2, "readable_text": True, "survey_status": "surveyed", "indexing_status": "indexed", "summary": None},
        },
        "created_at": "2026-01-01T00:00:00Z",
        "updated_at": "2026-01-01T00:00:00Z",
        "committed_at": "2026-01-01T00:00:00Z",
        "summary": "Legacy committed index.",
    }
    index_path.parent.mkdir(parents=True, exist_ok=True)
    index_path.write_text(json.dumps(v2, indent=2) + "\n", encoding="utf-8")
    before = index_path.read_bytes()

    inspected = runtime.material.inspect_source_index_schema(tmp_path)
    assert inspected.ok and inspected.value is not None
    assert inspected.value.stored_schema_version == 2
    assert inspected.value.migration_required
    assert index_path.read_bytes() == before
    ordinary_read = runtime.material.get_source_index(tmp_path)
    assert ordinary_read.ok and ordinary_read.value is not None
    assert ordinary_read.value.stored_schema_version == 2
    assert ordinary_read.value.migration_required
    assert index_path.read_bytes() == before

    changed_after_inspect = json.loads(before)
    changed_after_inspect["summary"] = "Changed after inspect."
    index_path.write_text(json.dumps(changed_after_inspect, indent=2) + "\n", encoding="utf-8")
    stale_inspection = runtime.material.migrate_source_index_schema(
        tmp_path,
        expected_source_index_digest=inspected.value.current_digest,
    )
    assert not stale_inspection.ok
    assert stale_inspection.issues[0].kind == "source_index_migration_digest_mismatch"
    assert json.loads(index_path.read_text(encoding="utf-8"))["summary"] == "Changed after inspect."
    index_path.write_bytes(before)

    with monkeypatch.context() as scoped:
        scoped.setattr(
            SourceIndexComponent,
            "_verify_migrated_source_index",
            lambda self, repo_root: self.runtime.foundation.fail(
                self.runtime.foundation.issue("injected_migration_verify_failure", "Injected verification failure.")
            ),
        )
        migrated = runtime.material.migrate_source_index_schema(
            tmp_path,
            expected_source_index_digest=inspected.value.current_digest,
        )
    assert not migrated.ok
    assert index_path.read_bytes() == before
    assert json.loads(index_path.read_text(encoding="utf-8"))["schema_version"] == 2

    original_fsync_parent = runtime.foundation.store._fsync_parent
    fsync_calls = 0
    ...
    def fail_first_parent_fsync(path: Path) -> None:
        ...
        fsync_calls += 1
        if fsync_calls == 1:
            raise OSError("injected post-replace parent fsync failure")
        original_fsync_parent(path)

    with monkeypatch.context() as scoped:
        scoped.setattr(runtime.foundation.store, "_fsync_parent", fail_first_parent_fsync)
        post_replace = runtime.material.migrate_source_index_schema(
            tmp_path,
            expected_source_index_digest=inspected.value.current_digest,
        )
    assert not post_replace.ok
    assert post_replace.issues[0].kind == "write_failed"
    assert index_path.read_bytes() == before
    assert json.loads(index_path.read_text(encoding="utf-8"))["schema_version"] == 2

    original_save_model = SourceIndexComponent._save_model
    source_file = tmp_path / ".lean_constellation" / "source" / "chapters" / "one.md"

    def mutate_source_then_save(self, repo_root: Path, index):
        source_file.write_text("Changed text A.\nChanged text B.\n", encoding="utf-8")
        return original_save_model(self, repo_root, index)

    with monkeypatch.context() as scoped:
        scoped.setattr(SourceIndexComponent, "_save_model", mutate_source_then_save)
        source_toc_tou = runtime.material.migrate_source_index_schema(
            tmp_path,
            expected_source_index_digest=inspected.value.current_digest,
        )
    assert not source_toc_tou.ok
    assert "source_index_source_hash_changed" in [issue.kind for issue in source_toc_tou.issues]
    assert index_path.read_bytes() == before
    assert json.loads(index_path.read_text(encoding="utf-8"))["schema_version"] == 2
    source_file.write_text("Definition A.\nTheorem B.\n", encoding="utf-8")

    def fail_restore(path: Path, payload: bytes):
        return runtime.foundation.issue(
            "source_index_migration_restore_failed",
            "Injected rollback failure.",
        )

    with monkeypatch.context() as scoped:
        scoped.setattr(runtime.foundation.store, "_fsync_parent", lambda path: (_ for _ in ()).throw(OSError("write failed")))
        scoped.setattr(SourceIndexComponent, "_restore_bytes_atomic", staticmethod(fail_restore))
        rollback_failure = runtime.material.migrate_source_index_schema(
            tmp_path,
            expected_source_index_digest=inspected.value.current_digest,
        )
    assert not rollback_failure.ok
    assert [issue.kind for issue in rollback_failure.issues] == [
        "write_failed",
        "source_index_migration_restore_failed",
    ]


def test_v2_draft_migration_is_explicitly_blocked_and_cannot_use_legacy_bridge(tmp_path: Path) -> None:
    runtime = _prepare_source(tmp_path)
    created = runtime.material.create_draft_source_index(tmp_path)
    assert created.ok
    path = tmp_path / ".lean_constellation" / "source_index" / "index.json"
    payload = json.loads(path.read_text(encoding="utf-8"))
    payload["schema_version"] = 2
    path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")

    restarted = make_runtime()
    inspected = restarted.material.inspect_source_index_schema(tmp_path)
    assert inspected.ok and inspected.value is not None
    assert "source_index_draft_migration_ambiguous" in inspected.value.blocking_issue_kinds
    migrated = restarted.material.migrate_source_index_schema(
        tmp_path,
        expected_source_index_digest=inspected.value.current_digest,
    )
    assert not migrated.ok and migrated.issues[0].kind == "source_index_draft_migration_ambiguous"
    bypass = restarted.material.set_source_index_overview(tmp_path, overview="Bypass")
...
def test_fresh_v3_unowned_draft_cannot_resume_after_runtime_rebuild(tmp_path: Path) -> None:
    ...
    restarted = make_runtime()
    block = restarted.material.create_source_block(
        tmp_path,
        parent_id="root",
        kind="statement",
...
def test_v2_committed_migration_succeeds_without_rewriting_semantic_ids(tmp_path: Path) -> None:
    ...
    payload["schema_version"] = 2
    payload.pop("active_update_id")
    payload.pop("active_file_scope")
    for file_payload in payload["files"].values():
    ...
    restarted = make_runtime()
    inspected = restarted.material.inspect_source_index_schema(tmp_path)
    assert inspected.ok and inspected.value is not None and not inspected.value.blocking_issue_kinds
    migrated = restarted.material.migrate_source_index_schema(
        tmp_path, expected_source_index_digest=inspected.value.current_digest
    )
...
def test_v2_inspection_reports_invalid_ref_ranges_and_link_graph_without_writing(tmp_path: Path) -> None:
    ...
    for file_payload in payload["files"].values():
        file_payload.pop("source_sha256")
        file_payload.pop("committed")
    payload["blocks"][block.block_id]["refs"][0]["material_ref"]["ref"]["end_line"] = 99
    payload["blocks"][block.block_id]["link_ids"] = ["link_bad"]
    payload["links"] = {
        "link_bad": {
            "link_id": "link_bad",
            "source_block_id": block.block_id,
            "target_block_id": None,
            "target_hint": "External target",
            "link_kind": "supports",
            "evidence_refs": [
                {
                    "kind": "source",
                    "ref": {"path": "chapters/two.md", "start_line": 1, "end_line": 1},
                }
            ],
            "created_at": "2026-01-01T00:00:00Z",
        }
    ...
    inspected = runtime.material.inspect_source_index_schema(tmp_path)
    assert inspected.ok and inspected.value is not None
    assert "source_ref_range_invalid" in inspected.value.blocking_issue_kinds
    assert "source_link_evidence_missing" in inspected.value.blocking_issue_kinds
    assert index_path.read_bytes() == before
    migrated = runtime.material.migrate_source_index_schema(
        tmp_path,
        expected_source_index_digest=inspected.value.current_digest,
