...
def test_inspect_mathlib_search_candidate_enriches_from_navigation_without_index_write(tmp_path: Path) -> None:
    ...
    inspected = service.inspect_mathlib_search_candidate(tmp_path, candidate_id=candidate_id)
    ...
    assert inspected.value.snippet == "theorem Nat.add_assoc : True := by trivial"
    assert not service.get_mathlib_decl_entry(tmp_path, name="Nat.add_assoc").ok
...
def test_inspect_mathlib_module_supports_module_name_alias_and_pattern_filter(tmp_path: Path) -> None:
    ...
    module = service.inspect_mathlib_module(tmp_path, module_name="Mathlib.Data.Finset.Basic", pattern="sum")
    ...
    assert module.value.important_decl_hints == ["Finset.sum_congr"]
