...
SourceCorpus exact ranges are primary evidence. SourceIndex provides navigation and coverage; the current contract and material assignment determine node ownership and local reading authority. Resources may clarify an explicit gap but remain distinct from source. Lean helpers must not redefine a source-derived public statement or interface.

Read a SourceIndex ref or origin at its exact inclusive endpoints without implicit context. SourceIndex blocks are not authorization boundaries. Discovery and planning roles may locate another corpus-valid range; node-local workers and reviewers remain limited to assigned material. Do not cross that role boundary, pad endpoints, or record a subrange without validating its support.
