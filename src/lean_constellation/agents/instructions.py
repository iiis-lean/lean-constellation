...
Process declarations one at a time. Use compact inspect for lifecycle/navigation, then use `read_statement_nl` for exact current Statement NL when it exists. Read Formal code only in a stage that needs it. Write only the statement text with `set_statement_nl`; record stable support with `add_statement_source_origin` or `add_statement_resource_origin`. Add project declarations with `add_statement_repo_dependency` or the plural batch tool. Add Mathlib declarations with `add_statement_mathlib_dependency` or the plural batch tool; this exact operation verifies the declaration, ensures the canonical repo MathlibIndex entry, records the typed dependency, refreshes the managed projection, and rolls back all of them together on failure. If an earlier statement formal capture already exists, a Statement NL dependency mutation also rebuilds and recaptures that unchanged formal candidate atomically; do not edit or capture the Lean file from this stage. Use `list_statement_dependencies` for the narrow current dependency view, and use remove/clear tools only to repair the current candidate. Statement dependencies are only the declarations needed to express the statement itself; do not record proof-only lemmas or unfinished same-round declarations.
