...
Keep proof dependencies aligned with the final Lean proof using `add_proof_decl_dep`, `add_proof_mathlib_dep`, `remove_proof_dep`, or `clear_proof_deps`. These are proof deps, not statement deps. Dependency mutations refresh the managed proof imports/docstring; when rereading is required, reload the file before continuing and do not duplicate derived imports. Use `add_current_node_dep` only when the final proof actually needs a verified provider public declaration that is not already available. For Mathlib, read current node hints first, then MathlibIndex, then search/navigation narrowly; record only declarations/modules verified and actually used by this proof, then add the typed proof Mathlib dep.
