...
Preserve a coherent authored source tree or authored single-file format. When you must transcribe PDF-only mathematical material, prefer a sectioned LaTeX tree for a complete paper, multiple top-level sections, or long theorem/proof clusters. Use one durable content file only for brief material with no natural subdivision; authored Markdown and brief prose-heavy material may remain Markdown.

Before submitting, inspect the whole durable tree with `scan_source_corpus`, run `check_source_corpus_draft`, perform the complete fidelity self-check, and repair failures in the same AgentStep. Confirm that final Markdown/LaTeX representation is syntactically intact as well as mathematically faithful. Submit ready only when the current tree is a coherent static SourceCorpus that an independent Reviewer can approve without relying on your summary. Submit blocked only when a critical requested input cannot be reliably located, accessed, identified, or read. After an accepted submit, stop.
...
Reject unexplained monolithic Markdown for a complete or otherwise non-brief multi-section mathematical work. For a multi-file candidate, verify its canonical entry, reading order, section boundaries, cross-file joins, numbering, bibliography, and cross-references as part of the complete fidelity pass.
...
For a local resource, work in the current active resource draft directory. Read and apply `faithful-material-preservation`, `pdf-faithful-transcription`, `material-fidelity-check`, and `resource-draft-curation`. Put downloads, originals, OCR, screenshots, extraction scratch, previews, and `draft.json` under `_work/`. Everything outside `_work/` is final-facing static material and is copied unchanged during finalization. Organize it with natural paths such as `article/`, `assets/`, or `supplementary/`, plus the required root `README.md`; do not include acquisition, extraction, Curator, or round history. Preserve authored structure; for a complete or long multi-section mathematical PDF, prefer a sectioned LaTeX tree, while a short narrow excerpt or brief prose-heavy item may use one `.tex` or Markdown entry. Complete the full fidelity self-check and run `check_resource_draft` before `submit_local_resource_created`.
