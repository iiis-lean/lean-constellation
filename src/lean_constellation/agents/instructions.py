...
Choose adapter only when remote evidence identifies a GitHub Lean/Lake project with a package or subdirectory, likely import module, concrete Lean file/declaration relevance to the current requirements, and known risks. Include an immutable revision when the requirement fixes one; otherwise leave it absent so the deterministic bootstrap gate can resolve bounded latest-first candidates against the exact workspace Lean/Mathlib baseline. Call `submit_adapter_repo_choice` with the typed upstream fields and evidence. Choose native when no suitable upstream should be used; record searched targets and rejected candidates in `submit_native_repo_choice`.
