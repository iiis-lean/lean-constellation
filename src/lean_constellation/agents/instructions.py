...
Use the requested/current configuration in `get_current_repo_run_context` and read `get_current_repo_work_config` separately only for a mismatch, configuration change, or explicit audit. Use the Coordinator mode skill matching the current work mode before making structural or task-planning decisions. When the current prompt names a required Skill, re-read that Skill from the current Home in this turn rather than relying on earlier memory:
...
Process declarations one at a time. Use compact inspect for lifecycle/navigation, then use `read_statement_nl` for exact current Statement NL when it exists. Read Formal code only in a stage that needs it. Write only the statement text with `set_statement_nl`; record stable support with `add_statement_source_origin` or `add_statement_resource_origin`; record all already-known statement-level project and Mathlib dependencies in one `add_statement_dependencies` batch, then reread once when requested. Use remove/clear tools only to repair the current candidate. Statement dependencies are only the declarations needed to express the statement itself; do not record proof-only lemmas or unfinished same-round declarations.
...
Keep statement dependencies aligned with the final formal statement using `add_statement_dependencies`, `remove_statement_dep`, or `clear_statement_deps`. Add already-known dependencies in one batch. These mutations refresh managed imports/docstrings; when the result requires rereading, reload the working file once after the batch before editing and do not recreate system-derived imports by hand. After capture, do not edit again unless you capture again. Before submit, verify `check_formal_stage_consistency` passes.
...
Process declarations one at a time. For each assigned theorem, inspect the accepted statement NL, captured statement formal code/check, current proof candidate, previous attempts and review feedback, compact source index coverage with `get_source_index_overview`, relevant source blocks with `list_source_blocks`/`get_source_block`, relevant source/resource text, visible declarations, current node contract, current Mathlib hints, and repo MathlibIndex. The proof route should name key lemmas, induction or case splits, dependency uses, known gaps, and any helper declaration that must exist.
...
Proof dependencies are proof-level dependencies, separate from statement dependencies. Record all already-known visible project and verified Mathlib proof dependencies in one `add_proof_dependencies` batch, then reread once when requested. Use remove/clear tools only to repair the current proof candidate. If current node dependencies are insufficient, first verify the provider public declaration, then use `add_current_node_dep` narrowly for that provider dependency and record the actual declaration with `add_proof_dependencies`. Do not delete existing node deps as part of ordinary proof work.
...
Keep proof dependencies aligned with the final Lean proof using `add_proof_dependencies`, `remove_proof_dep`, or `clear_proof_deps`. These are proof deps, not statement deps. Dependency mutations may refresh the managed proof imports/docstring. A successful result with `managed_projection_changed=true` or `reread_required=true` is expected and is not a blocker: reread the declaration-owned file once after the batch in the same AgentStep, then continue the current proof. Never submit blocked merely because rereading is required. Add already-known dependencies in one batch and do not duplicate derived imports. Use `add_current_node_dep` only when the final proof actually needs a verified provider public declaration that is not already available. For Mathlib, read current node hints first, then MathlibIndex, then search/navigation narrowly; record only declarations/modules verified and actually used by this proof, then add them through the typed proof dependency batch.
