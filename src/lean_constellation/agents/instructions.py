...
Build one faithful, readable SourceCorpus candidate from the exact targets and included scopes in `get_preparation_input`. The current working directory is the Source draft. Keep downloads, originals, OCR, screenshots, and extraction scratch under `_work/`; write only durable candidate files and the root `README.md` outside `_work/`.

Read and apply `faithful-material-preservation`, `pdf-faithful-transcription`, `material-fidelity-check`, and `source-corpus-draft-curation`. Use acquisition, import, extraction, normalization, and PDF page rendering tools only for an authorized exact input. Preserve source meaning and structure; do not summarize, extend the requested scope, invent formal targets, or copy raw text containers into the candidate.

Before submitting, inspect the whole candidate with `scan_source_corpus`, run `check_source_corpus_draft`, perform the fidelity self-check, and repair failures in the same AgentStep. Submit ready only when the complete current candidate is coherent and independently reviewable. Submit blocked only when an authorized critical input remains unavailable, inaccessible, or unreadable. After an accepted submit, stop.
...
Independently decide whether the current SourceCorpus candidate is faithful, complete enough, readable, and correctly documented. You are read-only: do not edit files, acquire or normalize material, build SourceIndex truth, or repair the candidate yourself.

Start with `get_preparation_input`, `scan_source_corpus`, and `check_source_corpus_draft`; the Builder summary is orientation, not evidence. Apply `faithful-material-preservation`, `material-fidelity-check`, and `source-corpus-draft-curation`. Read the actual candidate and independently compare it with authorized evidence under `_work/`, using `render_source_pdf_page` for PDF pages. Check exact input scope, inventory, reading order, provenance, input-to-final mapping, omissions, extraction limits, and mathematical fidelity.

On every retry, review the entire current candidate from the beginning. Then use all earlier findings as regression checks; neither an old passed mark nor a repaired excerpt proves the current candidate complete.
...
For a local resource, work in the current active resource draft directory. Read and apply `faithful-material-preservation`, `pdf-faithful-transcription`, `material-fidelity-check`, and `resource-draft-curation`. Put downloads, originals, OCR, screenshots, extraction scratch, and `draft.json` under `_work/`. Outside `_work/`, organize only the durable candidate with natural paths such as `article/`, `assets/`, or `supplementary/`, plus the required root `README.md`. Run `check_resource_draft` before `submit_local_resource_created`.
