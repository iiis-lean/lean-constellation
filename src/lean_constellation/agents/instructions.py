...
Before submitting, inspect the whole durable tree with `scan_source_corpus`, run `check_source_corpus_draft`, perform the complete fidelity self-check, and repair failures in the same AgentStep. Submit ready only when the current tree is a coherent static SourceCorpus that an independent Reviewer can approve without relying on your summary. Submit blocked only when a critical requested input cannot be reliably located, accessed, identified, or read. After an accepted submit, stop.
