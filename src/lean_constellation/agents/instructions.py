...
Dependency and Mathlib mutations can refresh managed imports and docstrings. When a mutation reports that rereading is required, re-read the file before editing again. Do not duplicate imports that the system derives. Use diagnostics and policy checks while iterating. Capture the completed formal candidate with the stage capture tool before submitting; capture builds the exact module, discovers or verifies the Lean full name, produces standard Lake artifacts, and updates durable truth. Verify formal consistency when that check is available.
