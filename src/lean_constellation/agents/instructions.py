...
Build or repair the repository's source target into a faithful, readable SourceCorpus candidate. Your working directory is the current source corpus root; direct file edits must stay inside that directory. Read and apply `faithful-material-preservation` and `source-corpus-faithful-preparation` before organizing durable material.
...
For PDF material, retain the original and use `render_source_pdf_page` to compare the readable transcription with the actual page; do not accept OCR text without checking high-risk formulas, symbols, ranges, cases, and references. On retry, read the reviewer feedback, repair it, inspect affected seams, and re-run the complete draft checks.

Before submitting, inspect with `scan_source_corpus` and run `check_source_corpus_draft`; repair gate failures in the same AgentStep. Call `submit_source_corpus_builder_ready` only after the current candidate is coherent and ready for independent review. Call `submit_source_corpus_builder_blocked` only when critical material is unavailable, inaccessible, or unreadable outside your authority. After an accepted ready or blocked submit, stop.
...
Start with `get_preparation_input`, `scan_source_corpus`, and `check_source_corpus_draft`; the Builder summary is only orientation. Use source search/range tools to read the actual current text. Apply `faithful-material-preservation` and `source-corpus-faithful-preparation`. Check inventory, reading order, canonical entry, provenance, author/version, retained-original mapping, included/omitted boundary, missing material, OCR/extraction limits, and correction ledger. Preserve an author's coherent TeX tree, macros, bibliography, assets, statements, formulas, and proof structure.

For retained PDFs, use `render_source_pdf_page` and compare the original pages independently with readable transcriptions. Check completeness and high-risk formula scope, quantifiers, primes, subscripts, relations, ranges, case endpoints, numbering, and cross-references. Do not approve from OCR, filenames, the correction ledger, or the Builder summary alone. On retry, perform a fresh pass over the entire current candidate, then treat earlier findings as regression checks.
