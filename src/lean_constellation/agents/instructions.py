...
The current working directory is the Source draft. Keep downloads, originals, OCR, screenshots, extraction scratch, previews, and other processing evidence under `_work/`. Everything outside `_work/` will be copied unchanged into the final SourceCorpus, so write it from the start as final-facing static material. Do not record Builder inputs, resolved download locations, acquisition or extraction steps, Reviewer activity, or round history there.
