...
6. For Statement NL workers, add project dependencies with the statement repo dependency tool and Mathlib dependencies with the statement Mathlib dependency tool. Use the singular form for one item and the plural form for a small known batch. If a previous statement formal capture exists, either dependency tool preserves unmanaged Lean source and atomically rebuilds and recaptures the unchanged candidate; the NL worker still does not edit or capture files. Use dependency removal or clearing only to repair the current candidate. Do not use a flat mixed dependency list or an untyped origin dictionary.
...
8. For Proof NL workers, write proof-route text first, then add only stable typed source/resource origins that support the proof route itself. Add project and Mathlib proof dependencies through their separate typed tools. If a previous proof formal capture exists, either dependency tool preserves unmanaged Lean source and atomically rebuilds and recaptures the unchanged candidate; the NL worker still does not edit or capture files. Do not use a flat mixed dependency list or an untyped origin dictionary.
...
The revision/reason remains structured truth and is not copied into the docstring. Formal capture requires every dependency to resolve to a module and full name. Dependency mutation refreshes managed docstrings and imports. In an NL stage, an existing capture is preserved by an atomic rebuild and recapture because the NL actor cannot edit or capture Lean files. A Formal worker retains explicit capture ownership and must re-read the file whenever the mutation result reports that rereading is required; do not hand-write an import that the managed projection derives.
