...
Bibliographic metadata normalization may change spelling, field completeness, or display style only when the resulting entry still identifies the same work cited by the source. It must not substitute a similar or unverified version, add an uncited work, or expand the requested included scope.
""",
    ),
    ...
    SkillKey.NODE_CONTRACT_DESIGN.value: LeanSkillDefinition(
        ...
        body=_body(
            ...
            (
                ...
                "Use the actual supplied Lean declarations and dependencies when available to calibrate the estimate. Prefer roughly 5–15 active tracked declarations per Content package, including definitions and private helpers; smaller independent packages and justified larger packages are valid. Put current scale-review and escalation guidance in execution_constraints, never a numeric quota in success criteria. Explicitly carry needed guidance into child plans; execution_constraints are not implicitly inherited.",
...
""",
    ...
    SkillKey.VISIBLE_NODE_DEPENDENCY_RECON.value: LeanSkillDefinition(
        ...
        body=_body(
            ...
            "Use this skill when comparing the current node's live work with stable same-repository or imported provider boundaries and recording a justified dependency.",
            (
                "Read the current contract, objective, and existing dependencies with `get_current_node_contract` and `list_current_node_deps`; the current node's public view is live planning context, not evidence that another node may consume it.",
                "Use `list_visible_nodes` and `list_imported_repos` to find provider boundaries. A same-repository Content provider is visible only through its active committed contract head; a Scope provider is visible only through its active committed exports; imported repositories use their stable published boundaries. Open-only or newer open candidates are not dependency evidence.",
                "Inspect provider declarations selectively with node/repo public declaration tools before recording a dependency, and preserve the exact stable revisions returned by those tools.",
                "Add dependencies with `add_current_node_dep` only when the stable boundary supports the node objective.",
    ...
    SkillKey.SCOPE_EXPORT_INTERFACE_CURATION.value: LeanSkillDefinition(
        ...
        body=_body(
            ...
            (
                "Read required interfaces and the requested Scope's current candidate with `list_node_interfaces` and `list_scope_exports`.",
                "Use `list_scope_export_candidates` to select only stable direct-child inputs: a Content child contributes exact public declarations from its active committed contract head, and a Scope child contributes exact refs from its active committed exports. Open child candidates are not export sources.",
                ...
                "If multiple stale exports block one another's removal, pass the other exact references in `remove_scope_export.additional_refs` to remove them atomically, then add the current public revisions. Do not change declaration visibility to repair a stale revision reference; bound interfaces and release compatibility must still be respected.",
                ...
                "For one reviewed Content declaration, use `revise_content_decl_visibility` with the observed visibility and an audit reason. Content-local visibility mutation uses that declaration's exact current committed revision and does not create or commit a Content contract revision.",
                "Use `promote_public_statement_closure` for an add-only Scope closure repair. Its declaration inputs must already belong to stable committed Content child boundaries. The requested target Scope must already have a caller-owned open revision; promotion updates that exact revision and never creates or commits the target. If the target has an active boundary but no open revision, open an owner edit explicitly before retrying.",
                "Promotion creates and commits only fresh add-only intermediate Scope revisions whose active boundaries have no open edits. An intermediate active-plus-open or open-only Scope is blocked; resolve it through its owner instead of merging unrelated edits.",
    ...
    SkillKey.PDF_FAITHFUL_TRANSCRIPTION.value: LeanSkillDefinition(
        name="pdf-faithful-transcription",
        description="Preserve a PDF's complete readable mathematical content while transcribing it into text.",
        group="material",
        ...
        body=_body(
            ...
            (
                ...
                "When choosing a representation for mathematical PDF content, LaTeX is the default durable representation. Preserve a coherent authored TeX, BibTeX, Markdown, or single-file format when one exists. When a complete or long paper must be reconstructed without authored source, prefer a sectioned LaTeX tree with an independent BibTeX bibliography and resolvable citation keys; this is a maintainability preference, not a hard requirement for brief material, material without a bibliography, or a faithful authored manual bibliography.",
                "Split at natural section or theorem/proof boundaries rather than fixed page counts. Keep one canonical entry with an explicit reading order, and do not add connective prose merely to make separate files read smoothly.",
                ...
                "Reliable Web evidence may complete DOI, URL, journal, volume, issue, page, or other metadata for a work that the PDF actually cites, but first verify that the metadata identifies that same work. Do not add uncited works or replace a citation with a merely similar title, author match, or unverified version; keep lookup evidence under `_work/` or runtime evidence.",
                "Bibliography fidelity is the identity and scope of the cited works, not byte-for-byte rendered numbering, ordering, punctuation, or style.",
            ...
            (
                ...
                "Do not summarize, paraphrase, translate, or improve the source unless the exact input explicitly requests that transformation.",
                "Do not place the PDF, OCR, screenshots, or extraction scratch in the final candidate tree.",
            ),
        ),
    ),
    SkillKey.MATERIAL_FIDELITY_CHECK.value: LeanSkillDefinition(
        ...
        body=_body(
            ...
            (
                ...
                "Validate the final representation as well as its meaning: Markdown/LaTeX math delimiters, environments, links, and other structural notation must remain balanced and readable rather than merely visually plausible in extracted text.",
                "For multi-file material, follow the canonical entry and reading order and check cross-file joins for omission or duplication. Across the whole tree, map each body citation to its bibliography entry and to the work actually cited by the source; numbering, ordering, punctuation, or bibliography style may differ without being a fidelity failure when that object identity and included scope remain unchanged.",
                "For reconstructed TeX, when the local toolchain is available, run the complete compilation closure appropriate to the tree, including BibTeX or Biber passes when used, and inspect the final logs for duplicate or undefined citations, bibliography keys, labels, and references. A first-pass PDF alone is not sufficient evidence.",
                "When the required TeX or bibliography toolchain is unavailable, record the unperformed checks and residual risk honestly rather than claiming that compilation or citation closure passed.",
            ...
            (
                "Do not approve because deterministic path and readability checks pass; they cannot establish mathematical fidelity.",
                "Do not narrow review to the latest reported edits or trust an earlier reviewer context as current evidence.",
                "Do not use a README warning as a substitute for repairing readable canonical content.",
            ),
        ),
    ),
    SkillKey.SOURCE_CORPUS_DRAFT_CURATION.value: LeanSkillDefinition(
        name="source-corpus-draft-curation",
        description="Define Source draft layout, README contract, full-candidate checks, review, and canonical projection.",
        group="source",
        required_tool_groups=_groups(
            AppGroup.SOURCE_CORPUS_READ,
        ...
        body=_body(
            ...
            (
                ...
                "Use natural durable paths such as `article/`, `article/sections/`, `assets/`, and `supplementary/` when they match the material. Do not create `original/` outside `_work/` merely to retain working evidence.",
    ...
    SkillKey.SOURCE_EVIDENCE_REFERENCING.value: LeanSkillDefinition(
        ...
        body=_body(
            ...
            (
                ...
                "Agents whose ToolView includes SourceCorpus text reading may inspect any corpus-valid range. Node owned/context refs identify responsibility and navigation; they do not restrict read access or expand Node/Decl mutation authority.",
                "Resolve source-defined variables, numbered objects, abbreviations, pronouns, and cross-references at their first relevant definition before judging a candidate. Keep the recorded origin exact; context read for interpretation is not automatically additional evidence.",
                "Record statement/proof source origins with the exact source path and both line endpoints, and keep the candidate, origin, and typed dependencies about the same mathematical object.",
            ),
            (
                ...
                "Do not treat previous reviewer feedback, old marks, or a worker summary as proof that the current candidate was fully checked.",
            ),
        ),
    ),
    ...
    SkillKey.RESOURCE_DRAFT_CURATION.value: LeanSkillDefinition(
        ...
        body=_body(
            ...
            (
                ...
                "Preserve a coherent authored tree or authored single-file format. For a complete or long multi-section mathematical Resource reconstructed from PDF, apply the shared PDF representation and citation preferences and use a LaTeX canonical entry split at natural sections or theorem/proof clusters; the independent BibTeX preference is not a hard requirement for a short focused excerpt, single theorem, bibliography-free item, or brief prose-heavy material.",
            ...
            (
                ...
                "Do not finalize `_work/`, raw textual containers, OCR scratch, or draft lifecycle metadata as Resource content.",
...
## Review Package Scale And Names

Prefer roughly 5–15 active tracked declarations per Content node, including definitions and private helpers and excluding deleted declarations or historical revisions. This is a planning target, not a quota: do not pad small independent packages or split solely to hit a number. Before accepting a larger package, examine separable mathematical objects, dependency stages, consumers, and independently reviewable outputs. A keep decision must explain concrete coupling and why splitting would not help; saying only "coherent package" is insufficient. Do not hide shared declarations in large proofs or untracked multi-declaration files to reduce the count.

On every Content task callback, and before expanding a contract or redispatching work, compare the current count, planned additions, remaining source stages, and original estimate. In the next-action loop, explicitly keep the bounded responsibility, assign work to an existing owner, or split an independent package. Preserve already implemented declarations and identities; a soft scale target does not authorize moving or deleting them.

Choose meaningful Scope layers for distinguishable mathematical areas, not to reach a fixed depth. Do not lift Defs/Theorems directories mechanically into semantic Nodes. Broad upper Scope names such as Foundations are acceptable; toward Content leaves, use increasingly specific names for the mathematical object, construction, or result. Judge the full path. Avoid catch-all leaves such as Basic, Base, Core, Helpers, or KeyLemma when their responsibility can be named precisely.
...
For an unexecuted initial plan with an open contract, use `preview_abandon_node_plan`, then `abandon_node_plan` with the concrete replanning reason if allowed. This withdraws the plan without claiming completion: the obsolete node and draft remain historical. Any declaration, strategy, round, or runtime history disqualifies this narrow operation, as do current inbound references, children, and Release protection. Do not fake a completed contract to bypass deletion guards. A preview is advisory; apply rechecks current truth.

After successful abandonment, the same path may be recreated as a Scope with `create_scope_node` and populated with focused Content children. The replacement has a new node identity and does not inherit old contracts. Content nodes cannot directly acquire children. Re-read the tree after each structural change; do not use this workflow to migrate executed nodes or repair references by silently reusing an old path.
...
For every terminal Content result, including ready results, compare the current active declaration count and remaining/planned work with the contract estimate. Record material growth and any independently nameable remaining responsibility. Carry this evidence into the next-action loop's keep/split/reuse decision before extending a contract or dispatching again; closeout itself does not restructure nodes.
...
At each round callback and before additions, compare the current active tracked declaration count, planned additions, remaining source stages, and the contract's rough range. The usual Content planning target is roughly 5–15 declarations including definitions and private helpers, not a hard limit. Keep an unchanged assessment brief; explain material growth and concrete coupling when retaining a larger package. Do not hide shared helpers to reduce the count. If new work forms an independent responsibility or crosses the owned boundary, finish safe current closeout and report the consumer, source stages, scale estimate, and candidate split to the Coordinator. Count alone does not justify a blocked result, and ContentPlan cannot create child nodes or expand its own contract boundary.
...
Small batches do not excuse indefinite growth of the owning node. Before adding targets, perform the whole-node scale review from `decl-strategy-planning`; carry only relevant execution guidance into this Round. Leave ownership changes to the Coordinator rather than absorbing an independent package through successive small rounds.
...
Before deletion, call `preview_decl_delete_closure` to inspect the exact same-node downstream closure and its public members. Deletion accepts only private declarations. When deleting a public declaration is genuinely intended, first use the ordinary visibility-revision workflow with the current observed visibility and a concrete audit reason; do not treat deletion as an implicit demotion. Re-read the preview after every visibility change, then call `delete_decls` with the complete exact closure. The mutation recomputes closure and blockers, so the preview is guidance rather than an authorization receipt. Never auto-remove a consumer, unbind an interface, alter a Scope export, or bypass release protection to force deletion. Accept any structured visibility or stable-boundary blocker and re-plan explicitly.
...
2. Inspect each reported private formal Statement prerequisite and confirm that it belongs to this Content node, is active, and has an exact current committed revision satisfying the repository proof policy.
3. For one deliberate change, call `revise_current_decl_visibility` with the visibility you just observed, the requested visibility, and a concrete reason. For the complete add-only same-node repair, call `promote_current_node_public_statement_closure`.
4. Re-read the mutation receipt and current closure. These operations update Decl visibility plus managed public projections; they do not create a Decl revision, Round, or Content contract version and do not commit the current Content contract. Therefore they are valid during the first open-only Content task once the exact current Decl revisions are committed and ready.
...
Apply `current-node-public-boundary-curation` before mutating visibility; its exact-current-revision eligibility and admitted-consumer blockers are authoritative. A visibility revision creates no Decl round, Decl revision, or Content contract version and never silently removes an interface or export. If another node's required declaration is not already public through its stable boundary, report a precise Coordinator blocker instead of expanding authority.
