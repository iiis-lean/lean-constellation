...
_MANAGED_PUBLICATION_IGNORE_ROOTS = (
    PurePosixPath(".lake"),
    PurePosixPath(".lhf"),
    PurePosixPath(".lean_constellation/restructure"),
...
def classify_repo_path(path: PurePosixPath) -> RepoPathClassification:
    ...
    if parts[0] == ".lhf":
        return RepoPathClassification(normalized, RepoPathClass.STATIC_VIEW)
    if parts[:2] == (".lean_constellation", "restructure"):
        return RepoPathClassification(normalized, RepoPathClass.OPERATIONAL_WORK)
    if parts[0] == ".git":
