...
class RepoMetadataComponent:
    ...
    def ensure_repo_model(self, repo_root: Path, *, main_node: str = "Main") -> ServiceResult[RepoModelView]:
        ...
        path = self.foundation.layout.repo_metadata_path(ctx)
        self.foundation.store.ensure_dir(path.parent)
        if path.exists():
            existing = self.foundation.store.read_json(path, RepoModel)
            if not existing.ok or existing.value is None:
                return self.foundation.fail(existing.issues)
            ...
            return self.foundation.ok(RepoModelView(repo_root=str(Path(repo_root)), main_node=main_node, created=False))

        model = RepoModel(main_node=main_node)
        written = self.foundation.store.write_json_atomic(path, model, mode=WriteMode.CREATE_ONLY)
        if not written.ok:
            return self.foundation.fail(written.issues)
        return self.foundation.ok(RepoModelView(repo_root=str(Path(repo_root)), main_node=main_node, created=True))

    def get_repo_model(self, repo_root: Path) -> ServiceResult[RepoModelView]:
        path = self.foundation.layout.repo_metadata_path(self._ctx(repo_root))
        loaded = self.foundation.store.read_json(path, RepoModel)
        ...
        return self.foundation.ok(RepoModelView(repo_root=str(Path(repo_root)), main_node=loaded.value.main_node))
    ...
    def get_repo_format(self, repo_root: Path) -> ServiceResult[RepoFormatView]:
        ...
        loaded = self.foundation.store.read_json(path, RepoFormatState)
        if not loaded.ok or loaded.value is None:
    ...
    def set_repo_format(
        ...
        *,
        ...
        if repo_format == RepoFormat.UNKNOWN:
            return self.foundation.fail(
                self.foundation.issue("invalid_repo_format", "repo_format must be native or adapter.")
            )
        reason = reason.strip()
        if not reason:
            return self.foundation.fail(self.foundation.issue("missing_reason", "set_repo_format requires a reason."))
        ...
        if not provider_ready.ok:
            return self.foundation.fail(provider_ready.issues)
    ...
    def get_repo_policy(self, repo_root: Path) -> ServiceResult[RepoPolicyView]:
        ...
        if not path.exists():
            return self.foundation.ok(RepoPolicyView(repo_root=str(Path(repo_root)), policy=RepoPolicy()))
        loaded = self.foundation.store.read_json(path, RepoPolicy)
        if not loaded.ok or loaded.value is None:
            return self.foundation.fail(loaded.issues)
        return self.foundation.ok(RepoPolicyView(repo_root=str(Path(repo_root)), policy=loaded.value))
        ...
        *,
        ...
        if readiness_policy is not None:
            if not readiness_policy.strip():
                return self.foundation.fail(
                    self.foundation.issue("policy_invalid", "readiness_policy must be non-empty.")
                )
        ...
        try:
            ...
            return self.foundation.fail(self.foundation.issue("policy_invalid", f"Invalid repo policy: {exc}"))
        written = self.foundation.store.write_json_atomic(self._repo_policy_path(repo_root), policy)
        if not written.ok:
            return self.foundation.fail(written.issues)
        return self.foundation.ok(RepoPolicyView(repo_root=str(Path(repo_root)), policy=policy))
    ...
    def get_repo_state_view(self, repo_root: Path) -> ServiceResult[RepoStateView]:
        ...
        if not policy.ok or policy.value is None:
            return self.foundation.fail(policy.issues)
        ctx = self._ctx(repo_root)
        ...
        return self.foundation.ok(
            RepoStateView(
                ...
                preparation_input_exists=self.foundation.layout.preparation_input_path(ctx).exists(),
    ...
    def get_provider_ready(self, repo_root: Path) -> ServiceResult[ProviderReadyState]:
        ...
        if not path.exists():
            return self.foundation.ok(ProviderReadyState())
        return self.foundation.store.read_json(path, ProviderReadyState)

    def set_provider_ready(self, repo_root: Path, *, summary: str) -> ServiceResult[ProviderReadyState]:
        summary = summary.strip()
        if not summary:
            return self.foundation.fail(self.foundation.issue("missing_summary", "Provider ready summary is required."))
        state = ProviderReadyState(ready=True, summary=summary)
        written = self.foundation.store.write_json_atomic(self._provider_ready_path(repo_root), state)
        if not written.ok:
            return self.foundation.fail(written.issues)
        return self.foundation.ok(state)
    ...
    def _repo_format_path(self, repo_root: Path) -> Path:
        return self.foundation.layout.constellation_root(self._ctx(repo_root)) / "repo_format.json"

    def _repo_policy_path(self, repo_root: Path) -> Path:
        return self.foundation.layout.constellation_root(self._ctx(repo_root)) / "repo_policy.json"

    def _provider_ready_path(self, repo_root: Path) -> Path:
        return self.foundation.layout.constellation_root(self._ctx(repo_root)) / "provider_ready.json"

    def _count_open_requirements(self, ctx: FoundationContext) -> tuple[int, list]:
        req_root = self.foundation.layout.requirements_root(ctx)
        if not req_root.exists():
            return 0, []
        loaded = self.foundation.store.list_json(req_root, RepoDependencyRequirement)
        if not loaded.ok or loaded.value is None:
