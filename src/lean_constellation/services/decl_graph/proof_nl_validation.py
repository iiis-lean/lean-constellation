...
def validate_proof_nl_candidate(
    ...
    *,
    ...
    for origin in origins:
        origin_validation = validate_proof_origin_ref(runtime, repo_root, origin=origin, decl_name=decl_name)
    ...
    dep_validation = validate_proof_deps(runtime, repo_root, node_path=node_path, round_id=round_id, decl_name=decl_name, deps=deps)
...
def validate_proof_deps(
    ...
    *,
    ...
    issues = []
    for dep in deps:
        issue = _validate_proof_dep(runtime, repo_root, node_path, round_ref_names, dep, decl_name)
...
def validate_proof_origin_ref(runtime: Any, repo_root: Path, *, origin: object, decl_name: str) -> ServiceResult[None]:
...
def proof_nl_validation_message(runtime: Any, repo_root: Path, *, node_path: str, round_id: str, decl_name: str) -> str | None:
    result = validate_proof_nl_candidate(runtime, repo_root, node_path=node_path, round_id=round_id, decl_name=decl_name)
    if result.ok:
        return None
    if result.issues:
        return str(getattr(result.issues[0], "message", None) or getattr(result.issues[0], "summary", None) or "Proof NL candidate failed validation.")
    return "Proof NL candidate failed validation."


def _validate_proof_origin(runtime: Any, repo_root: Path, origin: object, decl_name: str):
    kind, ref = _origin_kind_ref(origin)
    if kind == "source":
        if ref is None:
            return runtime.foundation.issue("proof_origin_invalid", "Proof NL source origin must include a stable source path or ref.", object_ref=decl_name)
        index = runtime.material.get_committed_source_index(repo_root)
        if not index.ok or index.value is None:
            return runtime.foundation.issue("proof_origin_source_index_missing", "Proof NL source origin requires a committed SourceIndex.", object_ref=decl_name)
        if not _source_origin_ref_exists(index.value, ref):
            return runtime.foundation.issue("proof_origin_source_missing", f"Proof NL source origin does not match committed SourceIndex: {ref}.", object_ref=decl_name)
        return None
    if kind == "resource":
        if ref is None:
            return runtime.foundation.issue("proof_origin_invalid", "Proof NL resource origin must include a stable resource key.", object_ref=decl_name)
        resource_key = ref.split("#", 1)[0].split(":", 1)[-1].strip()
        resource = _get_resource(runtime, repo_root, resource_key)
        if not resource.ok:
            return runtime.foundation.issue("proof_origin_resource_missing", f"Proof NL resource origin does not match an active resource: {ref}.", object_ref=decl_name)
        return None
    return runtime.foundation.issue("proof_origin_kind_invalid", f"Proof NL origin kind is not a stable source/resource origin: {kind}.", object_ref=decl_name)


def _validate_proof_dep(runtime: Any, repo_root: Path, node_path: str, round_ref_names: set[str], dep: object, decl_name: str):
    kind = _dep_kind(dep)
    dep_name = _dep_name(dep)
    if not dep_name:
        return runtime.foundation.issue("proof_dep_invalid", "Proof dependency is missing a declaration name.", object_ref=decl_name)
    if kind == "mathlib_decl":
        ...
        if not entry.ok or entry.value is None:
            return runtime.foundation.issue("proof_mathlib_dep_not_recorded", f"Proof Mathlib dependency is not recorded in MathlibIndex: {dep_name}.", object_ref=decl_name)
        dep_module = _dep_module(dep)
        if not dep_module:
            return runtime.foundation.issue("proof_mathlib_dep_module_missing", f"Proof Mathlib dependency must include a module or be filled from MathlibIndex: {dep_name}.", object_ref=decl_name)
        if entry.value.module and dep_module != entry.value.module:
            return runtime.foundation.issue(
                "proof_mathlib_dep_module_mismatch",
                f"Proof Mathlib dependency module does not match MathlibIndex for {dep_name}.",
        ...
        return None
    if kind != "repo_decl":
        return runtime.foundation.issue("proof_dep_kind_invalid", f"Proof dependency has unsupported dependency kind: {kind}.", object_ref=decl_name)
    ...
    if dep_repo:
        ...
        if not public.ok or public.value is None:
            return runtime.foundation.issue("proof_dep_not_visible", f"Proof dependency provider repo is not visible: {repo_key}.", object_ref=dep_name)
        if not any(item.ref.name == dep_name for item in public.value):
            return runtime.foundation.issue("proof_dep_not_visible", f"Proof dependency is not visible on provider repo public interface: {dep_name}.", object_ref=dep_name)
        return None
    ...
    if dep_node not in {"", "Main", node_path}:
        ...
        if not public.ok or public.value is None:
            return runtime.foundation.issue("proof_dep_not_visible", f"Proof dependency provider node is not visible: {dep_node}.", object_ref=dep_name)
        if not any(item.ref.name == dep_name for item in public.value):
            return runtime.foundation.issue("proof_dep_not_visible", f"Proof dependency is not visible on provider node public interface: {dep_name}.", object_ref=dep_name)
        return None
...
def _dep_module(dep_item: object) -> str | None:
    if isinstance(dep_item, str):
        return None
    ref = dep_item.get("ref") if isinstance(dep_item, dict) else getattr(dep_item, "ref", None)
    raw_module = ref.get("module") if isinstance(ref, dict) else getattr(ref, "module", None)
    return str(raw_module).strip() if raw_module is not None and str(raw_module).strip() else None
