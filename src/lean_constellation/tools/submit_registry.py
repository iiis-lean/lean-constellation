...
def build_submit_tool_specs() -> list[ToolSpec]:
    ...
    specs = [
        ...
        _submit_tool(
            name="submit_source_corpus_builder_ready",
            description="Submit the current source corpus candidate for independent review; this does not finalize the manifest.",
            args_model=SubmitSourceCorpusBuilderReadyArgs,
            groups={SubmitGroup.SOURCE_CORPUS_BUILDER_SUBMIT},
            roles={"worker", "admin"},
            handler=handlers.submit_source_corpus_builder_ready,
        ),
        _submit_tool(
            name="submit_source_corpus_builder_blocked",
            description="Submit that source corpus preparation cannot continue without external action.",
            args_model=SubmitSourceCorpusBuilderBlockedArgs,
            groups={SubmitGroup.SOURCE_CORPUS_BUILDER_SUBMIT},
            roles={"worker", "admin"},
            handler=handlers.submit_source_corpus_builder_blocked,
        ),
        _submit_tool(
            name="submit_source_corpus_review",
            description="Submit one independent full-current SourceCorpus review decision.",
            args_model=SubmitSourceCorpusReviewArgs,
            groups={SubmitGroup.SOURCE_CORPUS_REVIEWER_SUBMIT},
            roles={"reviewer", "admin"},
            handler=handlers.submit_source_corpus_review,
        ),
        _submit_tool(
