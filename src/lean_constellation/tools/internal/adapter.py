...
def build_tool_specs() -> list[ToolSpec]:
    ...
    return [
        ...
        direct_tool(
            ...
            groups={AppGroup.ADAPTER_IMPORT_PREVIEW_READ},
            roles=roles,
        ),
        direct_tool(
            name="check_adapter_catalog_ready_preflight",
            description="Check the sole Adapter Catalog Agent submission gate, excluding Flow-owned projection and provider finalization.",
            args_model=NoArgs,
            capability=ToolCapability.READ,
            backing_service="adapter",
            backing_method="check_adapter_catalog_ready_preflight",
            result_view="gate_report",
            groups={AppGroup.ADAPTER_CATALOG_READY_READ},
            roles=roles,
        ),
    ]
